...
from nets.config import WEIGHT_DECAY
...
class Routing(layers.Layer):
    ...
    def get_config(self):
        ...
        config.update({
            'layers.GlobalAveragePooling2D()': self.avgpool,
            'layers.Dropout(rate=dropout_rate)': self.dropout,
            'layers.Dense(units=out_channels)': self.fc,
            'layers.Softmax()': self.softmax,
...
class CondConv2D(layers.Layer):
    ...
    def get_config(self):
        config = super().get_config().copy()
        config.update({
            'Routing(out_channels=num_experts, dropout_rate=0.2, name="routing_layer")': self.routing,

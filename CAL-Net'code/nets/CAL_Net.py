...
from nets.condconv import CondConv2D
from nets.yolox import darknet_body
...
def DCAM(inputs,input_shape,filter_num,reduction_ratio=0.5):
    #channel attention
    x=inputs
    ...
    dcamout = tf.multiply(channel_out, spatial_out)
    return dcamout
...
def CAL_Net(input_shape=(256, 256, 3), num_classes=2, backbone="yolox", phi='tiny', weight_decay=5e-4):
...
######################################################################################################################
    ...
    # P3=eca_block(P3)
    P3 = cbam_block(P3)
    #x2_1 = DCAM(P3, 64, 64) #yolox-l
    # x2_1 = DCAM(P3, 64, 96)#yolox-tiny
    # x2_2 = tf.multiply(x2_1, P3)
    # P3 = add([x2_2, P3])
    # P3__ = UpSampling2D(size=(4, 4))(P3)
    # P3__ = tf.image.resize(P3, [128, 128])
    # P3__ = Conv2D(channels[0], 1, activation='relu', padding='same', kernel_initializer=RandomNormal(stddev=0.02))(P3__)
